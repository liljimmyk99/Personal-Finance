//
//  Colors.swift
//  Personal Finance
//
//  Created by Jimmy Kane on 6/21/25.
//
import SwiftUI

struct PFColors {
    static let background: Color  = Color("PFBackground", bundle: .main)
    static let destructiveButton: Color  = Color("PFDestructiveButton", bundle: .main)
    static let headingText: Color  = Color("PFHeadingText", bundle: .main)
    static let mainText: Color  = Color("PFMainText", bundle: .main)
    static let primary: Color  = Color("PFPrimary", bundle: .main)
    static let signIn: Color  = Color("PFSignIn", bundle: .main)
    static let surface: Color  = Color("PFSurface", bundle: .main)
    static let textFieldActive: Color  = Color("PFTextFieldActive", bundle: .main)
    static let textFieldDisabled: Color  = Color("PFTextFieldDisabled", bundle: .main)
    static let textFieldStatic: Color  = Color("PFTextFieldStatic", bundle: .main)
}
